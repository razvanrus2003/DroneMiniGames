...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.75, 0.25, 0.25));
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, light_pos2);
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.1f));
        RenderMesh(meshes["sphere"], shaders["Simple"], modelMatrix);
    }
}
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    glUniform1i(is_spotlight_loc, is_spotlight);

    int light_pos2_loc = glGetUniformLocation(shader->program, "light_pos2");
    glUniform3f(light_pos2_loc, light_pos2.x, light_pos2.y, light_pos2.z);

    int light_angle_loc = glGetUniformLocation(shader->program, "cut_off");
    glUniform1f(light_angle_loc, lightAngle);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        glm::vec4 lightDirection4 = glm::vec4(lightDirection, 1);
        if (window->KeyHold(GLFW_KEY_Z)) lightDirection4 = transform3D::RotateOX(deltaTime) * lightDirection4;
        if (window->KeyHold(GLFW_KEY_X)) lightDirection4 = transform3D::RotateOX(-deltaTime) * lightDirection4;

        lightDirection = glm::vec3(lightDirection4);
    }
}
